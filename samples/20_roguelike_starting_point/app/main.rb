...
class Game
  ...
  def render_enemies
    ...
    state.enemies.map do |e| # for each enemy in the collection
      ...
      if e.is_dead # if enemy is dead
        alpha = 255 * state.tick_count.perc_to_zero(e.attacked_at, 30) # change opacity of enemy based on current frame and when it was attacked
      end
